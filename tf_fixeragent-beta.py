...
class TerraformCheckovFixer:
    ...
    def _process_check(self, check: Dict) -> Optional[Tuple[Path, str, str, str]]:
        ...
        # e.g., "module.snapshot_lifecycle.aws_lambda_function.delete_snapshot_lambda"
        # becomes "aws_lambda_function" and "delete_snapshot_lambda"
        resource_parts = resource.split(".")
        
        # Remove module prefix(es) if present
        if resource_parts[0] == "module":
            # Skip module name(s) - e.g., module.name.resource_type.resource_name
            # Find where the actual resource starts (after all module prefixes)
            idx = 1
            while idx < len(resource_parts) and resource_parts[idx] != resource_parts[idx]:
                idx += 1
        ...
        if not original_block:
            ...
            self.logger.debug(f"     Full resource identifier was: {resource}")
        ...
        for attempt in range(self.max_retries):
            try:
                prompt = self._build_intelligent_prompt(
                    ...
                    original_block,
                    fix_snippet,
                    tf_content,  # Full file context for references
                    attempt
                )
                ...
                if is_valid:
                    self.logger.info(f"  ✅ Fix validated successfully")
                    break
                else:
    ...
    def _get_safe_relative_path(self, file_path: Path) -> Path:
        ...
        Returns relative path if possible, otherwise returns a safe representation.
        """
        try:
            return file_path.relative_to(self.tf_root)
        except ValueError:
            # File is outside tf_root (e.g., in a module directory)
            # Try to get relative to a common parent
            try:
                # Find common parent
                common = Path(*[p for p in self.tf_root.parts 
                               if p in file_path.parts])
                if common != Path('.'):
                    return file_path.relative_to(common)
            except:
                pass
            
            # Fallback: return just the filename with parent dirs
            return Path(*file_path.parts[-3:]) if len(file_path.parts) >= 3 else file_path.name

    def _validate_block(self, block: str, resource_type: str, resource_name: str) -> Tuple[bool, str]:
        ...
        if not re.search(pattern, block):
            return False, f"Resource declaration mismatch"
    ...
    def _build_intelligent_prompt(
        ...
        # Extract variable references, data sources, locals from original
        variables = re.findall(r'var\.[a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*', original_block)
        data_sources = re.findall(r'data\.[a-zA-Z_][a-zA-Z0-9_]*\.[a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*', original_block)
        locals_refs = re.findall(r'local\.[a-zA-Z_][a-zA-Z0-9_]*', original_block)
        resource_refs = re.findall(r'[a-z_]+\.[a-zA-Z_][a-zA-Z0-9_]*\.[a-zA-Z_][a-zA-Z0-9_]*', original_block)
        
        references_context = ""
        if variables or data_sources or locals_refs or resource_refs:
            references_context = "\n\nCRITICAL - EXISTING REFERENCES TO PRESERVE:\n"
            if variables:
                references_context += f"Variables: {', '.join(set(variables))}\n"
            if data_sources:
                references_context += f"Data sources: {', '.join(set(data_sources))}\n"
            if locals_refs:
                references_context += f"Locals: {', '.join(set(locals_refs))}\n"
            if resource_refs:
                references_context += f"Resources: {', '.join(set(resource_refs))}\n"
        
        retry_note = ""
        if attempt > 0:
            retry_note = f"\n\n⚠️ RETRY ATTEMPT {attempt + 1}: Previous output was invalid.\nEnsure proper Terraform syntax with balanced braces."
        
        return f"""You are an expert Terraform security engineer. Your task is to apply a Checkov security fix to a Terraform resource while preserving ALL existing configuration.

CRITICAL REQUIREMENTS:
1. Output ONLY the complete Terraform resource block - nothing else
2. Start with: resource "{resource_type}" "{resource_name}"
3. Preserve EVERY existing attribute, block, and comment from the original
4. Preserve ALL variable references (var.xxx), data sources (data.xxx), locals (local.xxx)
5. Preserve ALL resource dependencies and references
6. Add or modify ONLY what's required by the security fix
7. Maintain proper HCL syntax with balanced braces
8. Use 2-space indentation consistently
9. NO explanations, markdown fences, or extra text
...
ORIGINAL RESOURCE BLOCK (preserve everything exactly):
{original_block}
{references_context}

REQUIRED SECURITY FIX FROM CHECKOV DOCS (add/modify these attributes):
{fix_snippet}

MERGE STRATEGY:
1. Start with the complete original resource structure
2. Identify which security attributes are missing or need modification
3. Add the missing security attributes in the appropriate location
4. If an attribute needs modification, update it carefully
5. Keep ALL existing attributes, references, and dependencies intact
6. Maintain the exact same indentation style
7. Ensure all braces are balanced{retry_note}
...
OUTPUT THE COMPLETE MERGED RESOURCE BLOCK NOW:"""
    ...
    def _extract_resource(
        ...
        if not match:
            # Debug: show what resources ARE in the file
            self.logger.debug(f"     Could not find resource \"{resource_type}\" \"{resource_name}\"")
            self.logger.debug(f"     Resources found in file:")
            found_resources = re.findall(r'resource\s+"([^"]+)"\s+"([^"]+)"\s*{', content)
            for r_type, r_name in found_resources[:10]:  # Show first 10
                self.logger.debug(f"       - {r_type}.{r_name}")
            if len(found_resources) > 10:
                self.logger.debug(f"       ... and {len(found_resources) - 10} more")
            return None
    ...
    def _generate_review_files(self, all_changes: Dict[Path, List[Tuple]]) -> None:
        ...
        for file_path, changes in all_changes.items():
            try:
                ...
                # Create output file path - handle files outside tf_root
                ...
                # Generate summary
                self._generate_summary(file_path, changes, output_file)
    ...
    def _generate_summary(
        self, original_file: Path, changes: List[Tuple], output_file: Path
    ) -> None:
        """Generate summary file for review."""
        relative_path = self._get_safe_relative_path(original_file)
        
        summary_lines = [
            "=" * 70,
            "TERRAFORM CHECKOV FIX SUMMARY",
            "=" * 70,
            f"Original: {relative_path}",
            f"Fixed:    {output_file.relative_to(self.output_dir)}",
            f"Date:     {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            "=" * 70,
            "",
            f"Security Fixes Applied: {len(changes)}",
            "",
        ]
        
        for _, _, check_id in changes:
            summary_lines.append(f"  • {check_id}")
        ...
        summary_lines.extend([
            ...
            "REVIEW STEPS:",
            "=" * 70,
            "1. Check the .diff file for all changes",
            "2. Verify security fixes are correctly applied",
            "3. Ensure all variable/data/local references are preserved",
            "4. Run: terraform fmt <file>",
            "5. Run: terraform validate",
            "6. Run: terraform plan",
            "7. If satisfied, copy fixed file to replace original",
            "",
            "=" * 70,
            "IMPORTANT:",
            "=" * 70,
            "• All fixes are AI-generated - always review carefully",
            "• Check that dependencies and references are intact",
    ...
    def _generate_report(self) -> Dict[str, int]:
        ...
        if not self.merge_directly and not self.dry_run and stats['success'] > 0:
            self.logger.info(f"\n📁 Review files saved to: {self.output_dir}")
            self.logger.info(f"📊 Check .diff files for detailed changes")
            self.logger.info(f"📝 Read .summary.txt files for review steps")
...
# Tool:       terraform_checkov_fixer.py
#
# ⚠️  WARNING: AI-generated fix - Review before production
# Verify all references and dependencies are preserved
# {"=" * 68}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Intelligent Terraform Checkov Auto-Fixer with LLM",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
HOW IT WORKS:
1. Reads Checkov JSON to identify which files/resources need which fixes
2. Looks up fix snippets from terraform_fixes/ directory (e.g., CKV_AWS_8.tf)
3. Uses LLM to intelligently merge fixes while preserving all Terraform references
4. Handles modules outside tf-root directory automatically
5. Generates review files or applies directly based on your choice

EXAMPLES:
  # Safe review mode (recommended)
  python terraform_checkov_fixer.py \\
    --tf-root ./terraform \\
    --failed-json checkov_results.json
...
FEATURES:
  • Handles resources in modules (even outside tf-root)
  • Preserves all variable, data, local, and resource references
  • Creates timestamped backups before applying changes
  • Generates diffs and summaries for review
  • Validates generated Terraform syntax

PREREQUISITES:
  1. Install Ollama: curl -fsSL https://ollama.ai/install.sh | sh
  2. Pull model: ollama pull llama3
  3. Run Checkov: checkov -d ./terraform --framework terraform -o json > checkov_results.json
  4. Create terraform_fixes/ directory with fix snippets from Checkov docs
        """
    )
    ...
    parser.add_argument(
        "--tf-root",
        required=True,
        help="Root directory of Terraform code"
    )
    parser.add_argument(
        "--failed-json",
        required=True,
        help="Checkov failed checks JSON file"
    )
    parser.add_argument(
        "--fixes-dir",
        default="terraform_fixes",
        help="Directory with Checkov fix snippets (default: terraform_fixes)"
    )
    parser.add_argument(
        "--output-dir",
        default="terraform_fixes_review",
        help="Output directory for review files (default: terraform_fixes_review)"
    )
    parser.add_argument(
        "--model",
        default="llama3",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Create timestamped backups (default: enabled)"
    )
    parser.add_argument(
        "--no-backup",
        action="store_false",
    ...
    parser.add_argument(
        "--merge-directly",
        action="store_true",
        help="Apply fixes directly to files (default: generate review files)"
    )
    parser.add_argument(
        "--max-retries",
        type=int,
        default=2,
        help="Max retry attempts for LLM (default: 2)"
    )
    parser.add_argument(
        "--log-level",
