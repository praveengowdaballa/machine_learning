...
def main():
    ...
    parser.add_argument('-ip', default='localhost', help='IP address of the Ollama server (default: localhost)')
    parser.add_argument('-p', default='11434', help='Ollama server port (default: 11434)')
    parser.add_argument('-m', default='llama2', help='Model name for Ollama (default: llama2)')
    parser.add_argument('-n', type=int, default=10, help='Number of lines to read from file (default: 10)')
    parser.add_argument('-o', default='log-analyzer-output.txt', help='Output file to save the response (default: log-analyzer-output.txt)')
    args = parser.parse_args()

    logging.info(
        "Parsed arguments: file=%s, config=%s, query=%s, ip=%s, port=%s, model=%s, lines=%d, output=%s",
        args.f, args.c, args.q, args.ip, args.p, args.m, args.n, args.o
    )
    ...
    # Incorporate user query if provided
    full_prompt = f"{base_prompt}\n\nUser Query: {args.q}\n\n" + "\n".join(last_n_lines) if args.q else f"{base_prompt}\n\n" + "\n".join(last_n_lines)
